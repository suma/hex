...
public:
    ...
    ulint length(uint field = 0) const;
    ...
    inline ulint position(uint node, uint field = 0) const {
        Q_ASSERT(field < Fragment::size_array_max);
        const Fragment *f = fragment(node);
        ulint offset = f->size_left_array[field];
        while (f->parent) {
            uint p = f->parent;
            f = fragment(p);
            if (f->right == node)
    ...
    inline ulint sizeRight(uint node, uint field = 0) const {
        Q_ASSERT(field < Fragment::size_array_max);
        ulint sr = 0;
        const Fragment *f = fragment(node);
        node = f->right;
    ...
    inline ulint sizeLeft(uint node, uint field = 0) const {
        Q_ASSERT(field < Fragment::size_array_max);
        return fragment(node)->size_left_array[field];
    }


    inline ulint size(uint node, uint field = 0) const {
        Q_ASSERT(field < Fragment::size_array_max);
        return fragment(node)->size_array[field];
    }

    inline void setSize(uint node, ulint new_size, uint field = 0) {
        Q_ASSERT(field < Fragment::size_array_max);
        Fragment *f = fragment(node);
        ulint diff = new_size - f->size_array[field];
    ...
    uint findNode(ulint k, uint field = 0) const;
...
template <class Fragment>
uint QFragmentMapData<Fragment>::findNode(ulint k, uint field) const
{
    Q_ASSERT(field < Fragment::size_array_max);
    uint x = root();
...
template <class Fragment>
uint QFragmentMapData<Fragment>::insert_single(ulint key, ulint length)
{
...
template <class Fragment>
ulint QFragmentMapData<Fragment>::length(uint field) const {
    uint root = this->root();
...
public:
    ...
    Iterator find(ulint k, uint field = 0) { return Iterator(this, data.findNode(k, field)); }
    ConstIterator find(ulint k, uint field = 0) const { return ConstIterator(this, data.findNode(k, field)); }

    uint findNode(ulint k, uint field = 0) const { return data.findNode(k, field); }
    ...
    inline ulint position(uint node, uint field = 0) const { return data.position(node, field); }
    inline uint next(uint n) const { return data.next(n); }
    inline uint previous(uint n) const { return data.previous(n); }
    inline ulint size(uint node, uint field = 0) const { return data.size(node, field); }
    inline void setSize(uint node, ulint new_size, uint field = 0)
